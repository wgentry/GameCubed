...
/*******************************************************************************
* Function Name: I2CM_1_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(I2CM_1_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef I2CM_1_I2C_ISR_ENTRY_CALLBACK
    I2CM_1_I2C_ISR_EntryCallback();
#endif /* I2CM_1_I2C_ISR_ENTRY_CALLBACK */

#if (I2CM_1_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = I2CM_1_I2C_ACK_ADDR;
#endif /* (I2CM_1_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */

    endTransfer = 0u; /* Continue active transfer */

    /* Calls customer routine if registered */
    if(NULL != I2CM_1_customIntrHandler)
    {
        I2CM_1_customIntrHandler();
    }

    if(I2CM_1_CHECK_INTR_I2C_EC_MASKED(I2CM_1_INTR_I2C_EC_WAKE_UP))
    {
        /* Mask-off after wakeup */
        I2CM_1_SetI2CExtClkInterruptMode(I2CM_1_NO_INTR_SOURCES);
    }
    ...
    */
    if(I2CM_1_CHECK_I2C_FSM_MASTER)
    {
        #if(I2CM_1_I2C_MASTER)
        {
            ...
            */
            if(I2CM_1_CHECK_INTR_MASTER_MASKED(I2CM_1_INTR_MASTER_I2C_BUS_ERROR))
            {
                I2CM_1_mstrStatus |= (uint16) (I2CM_1_I2C_MSTAT_ERR_XFER |
                                                         I2CM_1_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = I2CM_1_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(I2CM_1_CHECK_INTR_MASTER_MASKED(I2CM_1_INTR_MASTER_I2C_ARB_LOST))
            {
                I2CM_1_mstrStatus |= (uint16) (I2CM_1_I2C_MSTAT_ERR_XFER |
                                                         I2CM_1_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = I2CM_1_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(I2CM_1_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(I2CM_1_CHECK_I2C_MASTER_CMD(I2CM_1_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    I2CM_1_mstrStatus |= (uint16) (I2CM_1_I2C_MSTAT_ERR_XFER |
                                                             I2CM_1_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                I2CM_1_mstrStatus |= (uint16) I2CM_1_GET_I2C_MSTAT_CMPLT;
                ...
                #if(I2CM_1_I2C_MULTI_MASTER_SLAVE)
                {
                    if(I2CM_1_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(I2CM_1_CHECK_I2C_MASTER_CMD(I2CM_1_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            I2CM_1_I2C_MASTER_CLEAR_START;

                            endTransfer = I2CM_1_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!I2CM_1_CHECK_INTR_SLAVE_MASKED(I2CM_1_INTR_SLAVE_I2C_BUS_ERROR))
                               && I2CM_1_CHECK_INTR_MASTER_MASKED(I2CM_1_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = I2CM_1_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            I2CM_1_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            I2CM_1_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                            ...
                            I2CM_1_SetRxInterruptMode(I2CM_1_NO_INTR_SOURCES);
                        ...
                        else
                        {
                            ...
                            I2CM_1_state = I2CM_1_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (I2CM_1_CHECK_I2C_FSM_SLAVE) */
    {
        #if(I2CM_1_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or I2CM_1_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(I2CM_1_CHECK_INTR_SLAVE_MASKED(I2CM_1_INTR_SLAVE_I2C_BUS_ERROR |
                                                        I2CM_1_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(I2CM_1_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    I2CM_1_slStatus &= (uint8) ~I2CM_1_I2C_SSTAT_RD_BUSY;
                    I2CM_1_slStatus |= (uint8) (I2CM_1_I2C_SSTAT_RD_ERR |
                                                          I2CM_1_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    I2CM_1_slStatus &= (uint8) ~I2CM_1_I2C_SSTAT_WR_BUSY;
                    I2CM_1_slStatus |= (uint8) (I2CM_1_I2C_SSTAT_WR_ERR |
                                                          I2CM_1_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(I2CM_1_CHECK_I2C_FSM_MASTER)
    {
        #if(I2CM_1_I2C_MASTER)
        {
            ...
            */
            if(I2CM_1_CHECK_INTR_MASTER_MASKED(I2CM_1_INTR_MASTER_I2C_STOP))
            {
                I2CM_1_ClearMasterInterruptSource(I2CM_1_INTR_MASTER_I2C_STOP);
            ...
            {
                if(I2CM_1_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(I2CM_1_CHECK_INTR_MASTER_MASKED(I2CM_1_INTR_MASTER_I2C_NACK))
                    {
                        I2CM_1_ClearMasterInterruptSource(I2CM_1_INTR_MASTER_I2C_NACK);

                        I2CM_1_mstrStatus |= (uint16) (I2CM_1_I2C_MSTAT_ERR_XFER |
                                                                 I2CM_1_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(I2CM_1_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != I2CM_1_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                I2CM_1_SetTxInterruptMode(I2CM_1_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(I2CM_1_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(I2CM_1_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(I2CM_1_CHECK_INTR_RX_MASKED(I2CM_1_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  I2CM_1_mstrRdBufSize -
                                        (I2CM_1_mstrRdBufIndex + I2CM_1_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > I2CM_1_I2C_FIFO_SIZE)
                            {
                                diffCount = I2CM_1_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    I2CM_1_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                I2CM_1_mstrRdBufPtr[I2CM_1_mstrRdBufIndex] = (uint8)
                                                                                        I2CM_1_RX_FIFO_RD_REG;
                                I2CM_1_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(I2CM_1_CHECK_INTR_RX_MASKED(I2CM_1_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            I2CM_1_mstrRdBufPtr[I2CM_1_mstrRdBufIndex] = (uint8) I2CM_1_RX_FIFO_RD_REG;
                            I2CM_1_mstrRdBufIndex++;

                            if(I2CM_1_mstrRdBufIndex < I2CM_1_mstrRdBufSize)
                            {
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(I2CM_1_CHECK_INTR_MASTER_MASKED(I2CM_1_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            I2CM_1_mstrWrBufIndexTmp -= (I2CM_1_GET_TX_FIFO_ENTRIES +
                                                                   I2CM_1_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            I2CM_1_mstrWrBufIndex = I2CM_1_mstrWrBufIndexTmp;

                            I2CM_1_mstrStatus |= (uint16) (I2CM_1_I2C_MSTAT_ERR_XFER |
                                                                     I2CM_1_I2C_MSTAT_ERR_SHORT_XFER);

                            I2CM_1_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(I2CM_1_CHECK_INTR_TX_MASKED(I2CM_1_INTR_TX_EMPTY))
                        {
                            while(I2CM_1_I2C_FIFO_SIZE != I2CM_1_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(I2CM_1_mstrWrBufIndexTmp < I2CM_1_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(I2CM_1_mstrWrBufIndexTmp == (I2CM_1_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    I2CM_1_TX_FIFO_WR_REG = (uint32) I2CM_1_mstrWrBufPtr[I2CM_1_mstrWrBufIndexTmp];
                                    I2CM_1_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                            ...
                            I2CM_1_ClearTxInterruptSource(I2CM_1_INTR_TX_ALL);
                        #else
                            I2CM_1_ClearTxInterruptSource(I2CM_1_INTR_TX_EMPTY);
                        #endif /* (I2CM_1_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(I2CM_1_CHECK_INTR_TX_MASKED(I2CM_1_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            I2CM_1_mstrWrBufIndex = I2CM_1_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(I2CM_1_CHECK_I2C_MODE_NO_STOP(I2CM_1_mstrControl))
                    {
                        ...
                        I2CM_1_mstrStatus |= (uint16) (I2CM_1_I2C_MSTAT_XFER_HALT |
                                                                 I2CM_1_GET_I2C_MSTAT_CMPLT);

                        I2CM_1_state = I2CM_1_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
    ...
    } /* (I2CM_1_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(I2CM_1_CHECK_I2C_FSM_SLAVE)
    {
        #if(I2CM_1_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(I2CM_1_CHECK_INTR_SLAVE_MASKED(I2CM_1_INTR_SLAVE_I2C_NACK))
            {
                I2CM_1_ClearSlaveInterruptSource(I2CM_1_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (I2CM_1_GET_TX_FIFO_ENTRIES + I2CM_1_GET_TX_FIFO_SR_VALID);

                if(I2CM_1_slOverFlowCount > diffCount) /* Overflow */
                {
                    I2CM_1_slStatus |= (uint8) I2CM_1_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    I2CM_1_slRdBufIndexTmp -= (diffCount - I2CM_1_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                I2CM_1_slRdBufIndex = I2CM_1_slRdBufIndexTmp;

                /* Clean-up TX FIFO */
                I2CM_1_SetTxInterruptMode(I2CM_1_NO_INTR_SOURCES);
                I2CM_1_slOverFlowCount = 0u;
                I2CM_1_CLEAR_TX_FIFO;

                /* Complete master reading */
                I2CM_1_slStatus &= (uint8) ~I2CM_1_I2C_SSTAT_RD_BUSY;
                I2CM_1_slStatus |= (uint8)  I2CM_1_I2C_SSTAT_RD_CMPLT;
                I2CM_1_state     =  I2CM_1_I2C_FSM_IDLE;
                
            #ifdef I2CM_1_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                I2CM_1_I2C_SlaveCompleteCallback();
            #endif /* I2CM_1_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(I2CM_1_CHECK_INTR_SLAVE_MASKED(I2CM_1_INTR_SLAVE_I2C_WRITE_STOP))
            {
                I2CM_1_ClearSlaveInterruptSource(I2CM_1_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (I2CM_1_I2C_CTRL_REG & I2CM_1_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != I2CM_1_GET_RX_FIFO_ENTRIES)
                    {
                        #if(I2CM_1_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == I2CM_1_GET_RX_FIFO_ENTRIES) &&
                               (I2CM_1_CHECK_INTR_SLAVE_MASKED(I2CM_1_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        I2CM_1_slWrBufPtr[I2CM_1_slWrBufIndex] = (uint8) I2CM_1_RX_FIFO_RD_REG;
                        I2CM_1_slWrBufIndex++;
                    }

                    I2CM_1_DISABLE_SLAVE_AUTO_DATA;
                }

                if(I2CM_1_CHECK_INTR_RX(I2CM_1_INTR_RX_OVERFLOW))
                {
                    I2CM_1_slStatus |= (uint8) I2CM_1_I2C_SSTAT_WR_OVFL;
                }
                ...
                I2CM_1_slStatus &= (uint8) ~I2CM_1_I2C_SSTAT_WR_BUSY;
                I2CM_1_slStatus |= (uint8)  I2CM_1_I2C_SSTAT_WR_CMPLT;
                I2CM_1_state     =  I2CM_1_I2C_FSM_IDLE;
            ...
            */
        #if (I2CM_1_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
            if (I2CM_1_CHECK_INTR_SLAVE_MASKED(I2CM_1_INTR_SLAVE_I2C_ADDR_MATCH |
        ...
        #endif /* (I2CM_1_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                I2CM_1_ClearI2CExtClkInterruptSource(I2CM_1_INTR_I2C_EC_WAKE_UP);

                #if (I2CM_1_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != I2CM_1_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = I2CM_1_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) I2CM_1_RX_FIFO_RD_REG;
                        response = I2CM_1_I2C_ACK_ADDR;
                    }
            ...
            #if (I2CM_1_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == I2CM_1_I2C_NAK_ADDR)
                {
                #if (!I2CM_1_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    I2CM_1_DISABLE_INTR_SLAVE(I2CM_1_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2CM_1_CY_SCBIP_V0) */

                    /* Clear address match and stop history */
                    I2CM_1_ClearSlaveInterruptSource(I2CM_1_INTR_SLAVE_ALL);

                    /* ACK the address byte */
                    I2CM_1_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (I2CM_1_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    if(I2CM_1_CHECK_I2C_STATUS(I2CM_1_I2C_STATUS_S_READ))
                    /* TX direction: master reads from slave */
                    {
                        I2CM_1_SetTxInterruptMode(I2CM_1_INTR_TX_EMPTY);

                        /* Set temporary index to address buffer clear from API */
                        I2CM_1_slRdBufIndexTmp = I2CM_1_slRdBufIndex;

                        /* Start master reading */
                        I2CM_1_slStatus |= (uint8) I2CM_1_I2C_SSTAT_RD_BUSY;
                        I2CM_1_state     = I2CM_1_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (I2CM_1_slWrBufSize - I2CM_1_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == I2CM_1_I2C_FIFO_SIZE)
                            {
                                ...
                                I2CM_1_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                I2CM_1_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        I2CM_1_slStatus |= (uint8) I2CM_1_I2C_SSTAT_WR_BUSY;
                        I2CM_1_state     = I2CM_1_I2C_FSM_SL_WR;
                    }
                    ...
                    I2CM_1_ENABLE_INTR_SLAVE(I2CM_1_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2CM_1_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    I2CM_1_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(I2CM_1_CHECK_INTR_RX_MASKED(I2CM_1_INTR_RX_FULL))
            {
                ...
                diffCount =  I2CM_1_slWrBufSize -
                            (I2CM_1_slWrBufIndex + I2CM_1_I2C_FIFO_SIZE);

                if(diffCount > I2CM_1_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = I2CM_1_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    I2CM_1_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                I2CM_1_ClearRxInterruptSource(I2CM_1_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = I2CM_1_RX_FIFO_RD_REG;

                if(I2CM_1_slWrBufIndex < I2CM_1_slWrBufSize)
                {
                    I2CM_1_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    I2CM_1_slWrBufPtr[I2CM_1_slWrBufIndex] = (uint8) diffCount;
                    I2CM_1_slWrBufIndex++;
                }
                ...
                I2CM_1_ClearRxInterruptSource(I2CM_1_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(I2CM_1_CHECK_INTR_TX_MASKED(I2CM_1_INTR_TX_EMPTY))
            {
                while(I2CM_1_I2C_FIFO_SIZE != I2CM_1_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(I2CM_1_slRdBufIndexTmp < I2CM_1_slRdBufSize)
                    /* Data from buffer */
                    {
                        I2CM_1_TX_FIFO_WR_REG = (uint32) I2CM_1_slRdBufPtr[I2CM_1_slRdBufIndexTmp];
                        I2CM_1_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        I2CM_1_TX_FIFO_WR_REG = I2CM_1_I2C_SLAVE_OVFL_RETURN;

                        if(I2CM_1_slOverFlowCount <= I2CM_1_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            I2CM_1_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef I2CM_1_I2C_ISR_EXIT_CALLBACK
    I2CM_1_I2C_ISR_ExitCallback();
#endif /* I2CM_1_I2C_ISR_EXIT_CALLBACK */
